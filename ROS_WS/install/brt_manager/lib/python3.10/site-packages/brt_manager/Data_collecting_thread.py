...
class TimeSynchronize(Node):
    ...
    def __init__(self):
        ...
        self.get_logger().warn("Data Collect Run Using thread...")

        self.shutdown_event = threading.Event()
        self.call_count = 0
        self.thread_call_count = 0
        ...
        self.SCEN_NAME = f'{self.SCEN_NAME}_{str(self.allow_time)}Thread'
    ...
    def process_messages(self):
        try:
                ...
                if self.thread_call_count % 5 == 0 :
                    ...
                    with self.lock:
                        if time_since_last_cam0 > 10:
                            self.get_logger().info("종료합니다 TIME OUT")
                            return
            ...
            self.get_logger().warn(f"Exception in process_messages: {str(e)}")

        finally:
            self.get_logger().info("Exiting process_messages thread")
    ...
    def process_cam0_message(self, cam0_time_ns, cam0_msg):
        ...
        self.get_logger().info(f"Camera0 {cam0_msg.header.stamp.sec} matched start")
        for index in TimeSynchronize.all_cameras.keys():
            if index == 0:
                continue
            else:
    ...
    def destroy_node(self):
        """Override destroy_node to ensure the processing thread is stopped"""
