...
class TimeSynchronize(Node):
    ...
    def __init__(self):
        ...
        self.get_logger().warn("Data Collecting Not Using thread...")
        ...
        self.allow_time = 0.5
        self.SCEN_NAME = 'scne16'
        self.saved = False
        self.SCEN_NAME = f'{self.SCEN_NAME}_{str(self.allow_time)}origin'
        self.base_path = f'dataset/collect/{self.SCEN_NAME}'
        self.camera_paths = {
    ...
    def process_messages(self):
        ...
        try:
            ...
            while rclpy.ok(): # ros2 TimeSync 노드가 생존동안.
                time.sleep(1)
                if not self.running:
                    return
    ...
    def find_closest_message(self, buffer, target_time_ns):
        ...
        closest_msg = min(
            buffer[left:right],
            key=lambda x: abs(x[0] - target_time_ns) # buffer[msg_ns , msg]
        )[1]
