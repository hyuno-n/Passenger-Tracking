...
class InComplete(Node):
    def __init__(self):
        ...
        self.door1_in_complete = False
        self.door2_in_complete = False
        self.door3_in_complete = False

        self.bridge = CvBridge()

        self.model = YOLO('yolov8x.pt')

        self.timer = self.create_timer(1, self.in_complete_pub)
        ...
        self.publisher_inout_count = self.create_publisher(Image, 'inout_count')
    ...
    def in_complete_pub(self):
        ...
        if sum(self.door1_cnt_3sec) == 0 and self.door1_in_complete is False:
            print("문 1 승차 완료")
            self.door1_in_complete = True

        if sum(self.door2_cnt_3sec) == 0 and self.door2_in_complete is False:
            print("문 2 승차 완료")
            self.door2_in_complete = True
        '''
        if sum(self.door3_cnt_3sec) == 0 and self.door3_in_complete is False:
            print("문 3 승차 완료")
            self.door3_in_complete = True
        '''
        if self.door1_in_complete is True and self.door2_in_complete is True: # and self.door3_in_complete is True:
            print("모든 문 승차 완료")
